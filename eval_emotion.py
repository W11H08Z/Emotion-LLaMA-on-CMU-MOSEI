...
from minigpt4.datasets.datasets.MOSEI_dataset import MOSEI_Dataset
...
print(args.dataset)
if 'mosei_data_builder' in args.dataset:
    text_processor_cfg = cfg.datasets_cfg.mosei_data_builder.text_processor.train
    text_processor = registry.get_processor_class(text_processor_cfg.name).from_config(text_processor_cfg)
    vis_processor_cfg = cfg.datasets_cfg.mosei_data_builder.vis_processor.train
    vis_processor = registry.get_processor_class(vis_processor_cfg.name).from_config(vis_processor_cfg)

    eval_file_path = cfg.evaluation_datasets_cfg["mosei_data_builder"]["eval_file_path"]
    img_path = cfg.evaluation_datasets_cfg["mosei_data_builder"]["img_path"]
    batch_size = cfg.evaluation_datasets_cfg["mosei_data_builder"]["batch_size"]
    max_new_tokens = cfg.evaluation_datasets_cfg["mosei_data_builder"]["max_new_tokens"]
    print(eval_file_path)
    print(img_path)
    print(batch_size)
    print(max_new_tokens)

    data = MOSEI_Dataset(vis_processor, text_processor, img_path, eval_file_path)
    ...
    for batch in eval_dataloader:
        ...
        for j in range(len(answers)):
            ...
            if answers[j] not in ["happy", "sad", "anger", "surprise", "disgust", "fear"]:
